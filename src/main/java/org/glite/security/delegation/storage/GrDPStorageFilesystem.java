...
/**
 * This is the filesystem based implementation of the GrDPStorage interface. <br/>
 * 
 * <p>
 * It provides a local filesystem archival mechanism for delegated proxies
 * related information.
 * </p>
 * 
 * <p>
 * Storage is separated into two different areas: storage cache and actual
 * storage. The first one holds the temporary information during a delegation
 * request, including the certificate request, the private key and the list of
 * voms attributes expected in the proxy. The second holds the actual delegated
 * proxy information, including the private key and list of voms attributes (as
 * above) and the actual proxy.
 * </p>
 * 
 * <p>
 * The format inside the storage is: <br/>
 * 
 * <pre>
 ...
 * 
 * <p>
 * &lt;storage-base-path&gt; is taken from the delegationStorage property inside
 * dlgee.properties.
 * </p>
 * 
...
public class GrDPStorageFilesystem implements GrDPStorage {
	 ...
	 * 
	 * @param elem
	 *            Object containing the information about the delegation
	 *            request.
	 * @throws GrDPStorageException
	 *             Failed to store new delegation request in storage cache area.
	 */
	public void insertGrDPStorageCacheElement(GrDPStorageCacheElement elem) throws GrDPStorageException {
		...
		try {
			...
			writeToFile(elem.getDN(), elem.getDelegationID(), "userreq.pem", elem.getCertificateRequest().getBytes(),
					true);

			// Store private key
			writeToFile(elem.getDN(), elem.getDelegationID(), "userkey.pem", elem.getPrivateKey().getBytes(), true);
	 ...
	 * 
	 * In this case, as it is a filesystem based implementation, the operation
	 * is equivalent to an insertion, with the contents of certificate request,
	 * private key and list of voms attributes simply being replaced.
	 * 
	 * @param elem
	 *            Object containing the information about the delegation
	 *            request.
	 * @throws GrDPStorageException
	 *             Failed to storage new delegation request in storage cache
	 *             area.
	 */
	 ...
	 * 
	 * @param delegationID
	 *            The id of the delegation request to be returned.
	 * @param dn
	 *            The dn of the user owning the delegation request.
	 * @return The object containing the information on the delegation request.
	 * @throws GrDPStorageException
	 *             Could not retrieve a delegation request because either it
	 *             does not exist or an error occured while tried to access it.
	 */
	public GrDPStorageCacheElement findGrDPStorageCacheElement(String delegationID, String dn)
		...
		try {
			...
			elem.setVomsAttributes(GrDPX509Util.fromStringVOMSAttrs(readFromFile(dn, delegationID, "voms.attributes",
					true)));
	 ...
	 * 
	 * @param delegationID
	 *            The id of the delegation request to be deleted.
	 * @param dn
	 *            The dn of the owner of the delegation request.
	 * @throws GrDPStorageException
	 *             Failed to delete the delegation request as either it does not
	 ...
	 * @throws GrDPStorageException
	 *             Failed to storage new delegation proxy in storage area.
	 */
	public void insertGrDPStorageElement(GrDPStorageElement elem) throws GrDPStorageException {
		...
		try {
			// Store certificate
			writeToFile(elem.getDN(), elem.getDelegationID(), "userproxy.pem", elem.getCertificate().getBytes(), false);
			...
			writeToFile(elem.getDN(), elem.getDelegationID(), "termination.time",
					DateFormat.getDateTimeInstance(DateFormat.LONG, DateFormat.LONG).format(elem.getTerminationTime())
							.getBytes(), false);
	 ...
	 * 
	 * In this case, as it is a filesystem based implementation, the operation
	 * is equivalent to an insertion, with the contents of delegated
	 * certificate, private key and list of voms attributes simply being
	 * replaced.
	 * 
	 * @param elem
	 *            Object containing the information about the delegated proxy.
	 * @throws GrDPStorageException
	 *             Failed to store new delegated proxy in storage area.
	 */
	 ...
	 * 
	 * @param delegationID
	 *            The id of the delegated proxy to be returned.
	 * @param dn
	 *            The dn of the user owning the delegated proxy.
	 * @return The object containing the information on the delegated proxy.
	 * @throws GrDPStorageException
	 *             Could not retrieve a delegated proxy because either it does
	 *             not exist or an error occured while tried to access it.
	 */
	public GrDPStorageElement findGrDPStorageElement(String delegationID, String dn) throws GrDPStorageException {
		...
		try {
			...
			try {
				terminationTime = DateFormat.getDateTimeInstance(DateFormat.LONG, DateFormat.LONG).parse(
						readFromFile(dn, delegationID, "termination.time", false));
	 ...
	 * 
	 * @param delegationID
	 *            The id of the delegated proxy to be deleted.
	 * @param dn
	 *            The dn of the owner of the delegated proxy.
	 * @throws GrDPStorageException
	 *             Failed to delete the delegated proxy as either it does not
	 *             exist or could not be accessed.
	 */
	...
	/**
	 * Retrieves the contents of a given file from the storage/storage cache
	 * area.
	 * 
	 * Represents an abstraction from the actual filesystem structure for the
	 * storage areas.
	 * 
	 * @param dn
	 *            The dn of the owner of the wanted delegation request/proxy
	 *            information.
	 * @param dlgID
	 *            The delegation ID of the wanted delegation request/proxy
	 *            information.
	 * @param fileName
	 *            The actual file to be accessed (userproxy.pem, userreq.pem,
	 *            userkey.pem, voms.attributes).
	 * @param cache
	 *            True if storage cache area should be accessed. False if
	 *            storage area should be accessed.
	 * 
	 * @return The contents of the requested file.
	 * 
	 * @throws IOException
	 *             An error occurred while reading the contents of the file.
	 */
	 ...
	 * 
	 * @param dn
	 *            The dn of the owner of the delegation request/proxy
	 *            information.
	 * @param dlgID
	 *            The delegation ID of the delegation request/proxy information.
	 * @param fileName
	 *            The actual file where the contents should be written
	 *            (userproxy.pem, userreq.pem, userkey.pem, voms.attributes).
	 * @param cache
	 *            True if storage cache area should be accessed. False if
	 *            storage area should be accessed.
	 * 
	 * @throws IOException
	 *             An error occurred while writing the contents to the file.
	 */
	private void writeToFile(String dn, String dlgID, String fileName, byte[] content, boolean cache)
			throws IOException, GrDPStorageException {
		...
		if (new File(dlgDirPath).mkdirs()) {
			if (!GrDPX509Util.changeFileMode(dlgDirPath, 700)) {
				throw new GrDPStorageException("Failed to set read/write for owner only" + " on directory '"
						+ dlgDirPath + "'");
			}
		}
	 ...
	 * 
	 * If fileName given is null, the actual directory containing all the
	 * related stored credential files is removed.
	 * 
	 * @param dn
	 *            The dn of the owner of the delegation request/proxy
	 *            information.
	...
	private void removeFile(String dn, String dlgID, String fileName, boolean cache) throws IOException {
		...
		if (file.isDirectory()) {
			...
			if(!file.canWrite()){
				throw new IOException("Can't remove directory " + file.getPath() + " No write access.");
			}
			File[] files = file.listFiles();
			logger.debug("Num files inside: " + files.length);
			for (int i = 0; i < files.length; i++) {
				if(!files[i].canWrite()){
					throw new IOException("Can't remove directory " + files[i].getPath() + " No write access.");
				}
				boolean result = files[i].delete();
				if (!result) {
					throw new IOException("Failed to remove file '" + files[i].getAbsolutePath()
